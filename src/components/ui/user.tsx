...
const User: React.FC<UserProps> = ({ address }) => {
  ...
  return (
    <Link
      ...
      className='bg-tertiary border-primary relative flex max-w-full flex-row items-center gap-2 rounded-sm border px-1.5 py-1 transition hover:opacity-80'
    >
      ...
      />
      <p className='z-10 max-w-full truncate text-xl font-medium'>{profile?.ens?.name || truncateAddress(address)}</p>
    </Link>
  )
}

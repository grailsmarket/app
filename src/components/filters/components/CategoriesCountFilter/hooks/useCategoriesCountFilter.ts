...
export const useCategoriesCountFilter = () => {
  ...
  // CategoriesCountFilter is only rendered when clubsCount exists in the state
  const clubsCount = (selectors.filters as { clubsCount?: { min: number | null; max: number | null } }).clubsCount

  const minVal = clubsCount?.min ?? null
  const maxVal = clubsCount?.max ?? null
  ...
  const setMinClubs = (min: number) => {
    ...
    dispatch(actions.setClubsCount({ min: newMin, max: clubsCount?.max ?? null }))
  }

  const setMaxClubs = (max: number) => {
    const newMax = max === 0 ? null : minVal && max <= minVal ? minVal : max
    dispatch(actions.setClubsCount({ min: clubsCount?.min ?? null, max: newMax }))
  }

...
const TabSwitcher: React.FC<TabSwitcherProps> = ({ profileTab, setProfileTab }) => {
  ...
  if (!mounted) {
    return (
      <div className='px-md py-md md:px-lg border-tertiary w-full flex gap-4 border-b-2'>
        {profileTabs.map((tab) => (
          <button
            key={tab.value}
  ...
  return (
    <div className='px-md md:py-md md:px-xl border-tertiary w-full flex gap-4 border-b-2 py-0.5'>
      {profileTabs.map((tab) => (
        <button
          key={tab.value}
          onClick={() => setProfileTab(tab.value)}

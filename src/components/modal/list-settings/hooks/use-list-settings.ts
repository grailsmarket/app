import { fromHex, isAddress, type Chain, getContract, type Address, createPublicClient } from 'viem'
...
const useListSettings = ({ profile, list }: { profile: ProfileDetailsResponse; list: number }) => {
  ...
  const { data: roles, refetch: refetchRoles } = useQuery({
    queryKey: ['userRoles', connectedAddress, list],
    queryFn: async () => {
      if (!(connectedAddress && list))
        return {
          isOwner: true,
          isManager: true,
          isUser: true,
